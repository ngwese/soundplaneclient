...
template <class c> inline bool(within)(const c &x, const c &min, const c &max) {
  return ((x >= min) && (x < max));
}
...
inline float mapRange(const float a, const float b, const float c,
                      const float d, const float x) {
  return lerp(c, d, unlerp(a, b, x));
}

inline float mapAndClipRange(const float a, const float b, const float c,
                             const float d, const float x) {
  return lerp(c, d, clamp(unlerp(a, b, x), 0.f, 1.f));
}

// return city block distance between two touches with a "z importance"
// parameter. if z scale is too small, zero touches will get matched with new
// active ones in the same position. if too big, z is more important than
// position and nothing works.
...
TouchTracker::TouchTracker()
    : mSampleRate(1000.f), mMaxTouchesPerFrame(0), mLopassZ(50.),
      mRotate(false) {
  setThresh(0.1);
...
// set the threshold of curvature that will cause a touch. Note that this will
// not correspond with the pressure (z) values reported by touches.
...
void TouchTracker::setLopassZ(float k) { mLopassZ = k; }
...
SensorFrame TouchTracker::preprocess(const SensorFrame &in) {
  ...
  // filter out any negative values. negative values can show up from capacitive
  // coupling near edges, from motion or bending of the whole instrument, from
  // the elastic layer deforming and pushing up on the sensors near a touch.
  y = max(y, 0.f);

  // a lot of filtering is needed here for Soundplane A to make sure peaks are
  // in centers of touches. it also reduces noise. the down side is, contiguous
  // touches are harder to tell apart. a smart blob-shape algorithm can make up
  // for this later, with this filtering still intact.
...
// to clear the next frame, all touch z values must be set to 0 and states to
// kTouchStateOff so that the frame is guaranteed to be sent.
...
TouchArray TouchTracker::process(const SensorFrame &in, int maxTouches) {
  ...
  if (mMaxTouchesPerFrame > 0) {
    ...
    mTouches =
        filterTouchesZ(mTouches, mTouches2, mLopassZ * 2.f, mLopassZ * 0.25f);
    mTouches2 = mTouches;

    // after variable filter, exile decayed touches so they are not matched.
    // Note this affects match feedback!
...
float sensorToKeyY(float sy) {
  ...
  // NOTE: these y locations depend on the amount of smoothing done in
  // smoothPressureX / smoothPressureY.
  constexpr std::array<float, mapSize> sensorMap{
      {0.7, 1.2, 2.7, 4.3, 5.8, 6.3}};
...
Touch peakToTouch(Touch p) {
  return Touch{.x = mapRange(3.5f, 59.5f, 1.f, 29.f, p.x),
               .y = sensorToKeyY(p.y),
               .z = p.z};
}

// quick touch finder based on peaks of curvature.
// this works well, but a different approach based on blob sizes / shapes could
// do a much better job with contiguous keys.
TouchArray TouchTracker::findTouches(const SensorFrame &in) {
  ...
  for (j = 1; j < h - 1; ++j) {
    ...
    for (i = 1; i < w - 1; ++i) {
      ...
      f32 = pRow3[i];
      f33 = pRow3[1 + 1];
      row[i] = (f22 > f11) && (f22 > f12) && (f22 > f13) && (f22 > f21) &&
               (f22 > mFilterThreshold) && (f22 > f23) && (f22 > f31) &&
               (f22 > f32) && (f22 > f33);
    }
  }
  ...
  for (int j = 0; j < h; j++) {
    ...
    for (int i = 0; i < w; i++) {
      ...
      if (mapRow[i]) {
        float z = in[j * w + i];
        peaks[nPeaks++] = Touch{
            .x = static_cast<float>(i), .y = static_cast<float>(j), .z = z};
        if (nPeaks >= kMaxPeaks)
          break;
      }
    }
  ...
  if (nPeaks > 1) {
    std::sort(peaks.begin(), peaks.begin() + nPeaks,
              [](Touch a, Touch b) { return a.z > b.z; });
  }
...
// for each possible touch slot, output the touch x closest in location to the
// previous frame. if the incoming touch is a continuation of the previous one,
// set its age (w) to 1, otherwise to 0. if there is no incoming touch to match
// with a previous one at index i, and no new touch needs index i, the position
// at index i will be maintained.

// TODO first touch below filter threshold(?) is on one index, then active touch
// switches index?! investigate.

TouchArray TouchTracker::matchTouches(const TouchArray &x,
                                      const TouchArray &x1) {
  ...
  // fill in any free touches with previous touches at those indices. This will
  // allow old touches to re-link if not reused.
  for (int i = 0; i < mMaxTouchesPerFrame; ++i) {
...
// input: vec4<x, y, z, k> where k is 1 if the touch is connected to the
// previous touch at the same index.
//
TouchArray TouchTracker::filterTouchesXYAdaptive(const TouchArray &in,
                                                 const TouchArray &inz1) {
  // these filter settings have a big and sort of delicate impact on play feel,
  // so they are not user settable
  ...
  for (int i = 0; i < mMaxTouchesPerFrame; ++i) {
    ...
    if (age > 0) {
      ...
      float freq =
          mapAndClipRange(0., 0.02, kFixedXYFreqMin, kFixedXYFreqMax, z);
    ...
    out[i] = Touch{.x = newX, .y = newY, .z = z, .dz = 0, .age = age};
  }

  return out;
}

TouchArray TouchTracker::filterTouchesZ(const TouchArray &in,
                                        const TouchArray &inz1, float upFreq,
                                        float downFreq) {
  ...
  for (int i = 0; i < mMaxTouchesPerFrame; ++i) {
    ...
    // set state
    int newState = newGate ? (gate1 ? kTouchStateContinue : kTouchStateOn)
                           : (gate1 ? kTouchStateOff : kTouchStateInactive);

    out[i] = Touch{
        .x = x, .y = y, .z = newZ, .dz = dz, .age = newAge, .state = newState};
  }

  return out;
}

// if a touch has decayed below the filter threshold after z filtering, move it
// off the scene so it won't match to other nearby touches.
TouchArray TouchTracker::exileUnusedTouches(const TouchArray &preFiltered,
                                            const TouchArray &postFiltered) {
...
// rotate order of touches, changing order every time there is a new touch in a
// frame. side effect: writes to mRotateShuffleOrder
    ...
    if (doRotate) {
      ...
      if (nFree > 1) {
        ...
        for (int i = 0; i < nFree - 1; ++i) {
          mRotateShuffleOrder[freeIndexes[i]] =
              mRotateShuffleOrder[freeIndexes[i + 1]];
        }
...
TouchArray TouchTracker::getTestTouches(time_point<system_clock> now,
                                        int maxTouches) {
  TouchArray t{};
