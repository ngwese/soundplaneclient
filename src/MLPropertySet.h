...
// MLPropertySet: a Set of Properties. Property names are stored as keys to the
// property map.

// TODO with the Listener stuff this class wraps up two very different concepts:
// storage and communication. That seems like a bad design. communication should
// be a different class from the propertySet - a kind of decorator that does the
// sending maybe.
//
...
// think of change recorder as a kind of filter. Then transmitter is another
// generic thing that can be composed with it.

// FURTHERMORE: This is all starting to look like Tree< Property >.
...
public:
  ...
  template <typename T> void setProperty(ml::Symbol p, const T &v) {
  ...
  template <typename T> void setPropertyImmediate(ml::Symbol p, const T &v) {
    if (mAllowNewProperties) {
      mProperties[p].setValue(v);
  ...
  template <typename T>
  void setPropertyImmediateExcludingListener(ml::Symbol p, const T &v,
                                             MLPropertyListener *pL) {
    if (mAllowNewProperties) {
      mProperties[p].setValue(v);
...
private:
  ...
  void
  broadcastPropertyExcludingListener(ml::Symbol p, bool immediate,
                                     MLPropertyListener *pListenerToExclude);
};

// MLPropertyListeners are notified when a Property of an MLPropertySet changes.
// They do something in response by overriding doPropertyChangeAction().
...
public:
  ...
  // override to do whatever this PropertyListener needs to do based on the
  // values of properties.
  virtual void doPropertyChangeAction(ml::Symbol param,
                                      const ml::Value &newVal) = 0;

  // call periodically to do actions for any properties that have changed since
  // the last call.
...
protected:
  ...
  // if the property is new, or the value has changed, we mark the state as
  // changed. If immediate is true and the state has changed,
  // doPropertyChangeAction() will be called.
  void propertyChanged(ml::Symbol p, bool immediate);

  // Must be called by the Property owner to notify us in the event it is going
  // away.
  void propertyOwnerClosing();

  // PropertyStates represent the state of a single property relative to
  // updates.
  class PropertyState {
  public:
