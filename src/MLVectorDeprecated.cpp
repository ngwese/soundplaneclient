...
const MLV4 MLVec::kNullValue = {
    {kMLMinSample, kMLMinSample, kMLMinSample, kMLMinSample}};
...
MLVec MLVec::getFracPart() const { return *this - getIntPart(); }
...
bool MLVec::operator==(const MLVec &b) const {
  return (val.f[0] == b.val.f[0]) && (val.f[1] == b.val.f[1]) &&
         (val.f[2] == b.val.f[2]) && (val.f[3] == b.val.f[3]);
}

bool MLVec::operator!=(const MLVec &b) const { return !operator==(b); }
...
Vec2 intersect(const LineSegment &a, const LineSegment &b) {
  ...
  if (ml::sign(b3.start.y()) == ml::sign(b3.end.y()))
    return Vec2::null();

  float sectX = b3.end.x() + (b3.start.x() - b3.end.x()) * b3.end.y() /
                                 (b3.end.y() - b3.start.y());

  if (!ml::within(sectX, 0.f, aLen))
    return Vec2::null();
...
void MLRect::setToIntersectionWith(const MLRect &b) { *this = intersect(b); }

void MLRect::setToUnionWith(const MLRect &b) { *this = unionWith(b); }

void MLRect::translate(const Vec2 &b) { *this += b; }
...
void MLRect::centerInRect(const MLRect &b) { setCenter(b.getCenter()); }

MLRect MLRect::translated(const Vec2 &b) const { return *this + b; }

MLRect MLRect::withCenter(const Vec2 &b) const {
  return (
      translated(b - Vec2(left() + width() * 0.5f, top() + height() * 0.5f)));
}

MLRect MLRect::withCenter(const float cx, const float cy) {
  return (translated(
      Vec2(cx - left() - width() * 0.5f, cy - top() - height() * 0.5f)));
}
...
Vec2 MLRect::getTopLeft() const { return Vec2(left(), top()); }

Vec2 MLRect::getBottomRight() const { return Vec2(right(), bottom()); }

Vec2 MLRect::getDims() const { return Vec2(width(), height()); }
