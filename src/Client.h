...
public:
  ...
  // const ml::Matrix getRawSignal() { std::lock_guard<std::mutex>
  // lock(mRawSignalMutex); return mRawSignal; } const ml::Matrix
  // getCalibratedSignal() { std::lock_guard<std::mutex>
  // lock(mCalibratedSignalMutex); return sensorFrameToSignal(mCalibratedFrame);
  // }

  // const ml::Matrix getSmoothedSignal() { std::lock_guard<std::mutex>
  // lock(mSmoothedSignalMutex); return mSmoothedSignal; }
  ...
  // const std::vector< Zone >::const_iterator getZonesBegin(){ return
  // mZones.begin(); } const std::vector< Zone >::const_iterator getZonesEnd(){
  // return mZones.end(); }
