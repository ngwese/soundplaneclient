...
// mKymaMode(false)
{
  ...
  // MLConsole() << "SoundplaneModel: listening for OSC on port " <<
...
void SoundplaneModel::doPropertyChangeAction(ml::Symbol p,
                                             const ml::Value &newVal) {
  // std::cout << "SoundplaneModel::doPropertyChangeAction: " << p << " -> " <<
  // newVal << "\n";
...
void SoundplaneModel::onClose() { enableOutput(false); }
...
void SoundplaneModel::outputTouches(TouchArray touches,
                                    time_point<system_clock> now) {
...
void SoundplaneModel::sendControllerToOutputs(int zoneID, int offset,
                                              const ZoneMessage &m) {
  // if(mMIDIOutput.isActive())
  // {
...
//
// void SoundplaneModel::ProcessMessage(const osc::ReceivedMessage& m, const
...
// void SoundplaneModel::ProcessBundle(const osc::ReceivedBundle &b, const
// IpEndpointName& remoteEndpoint)
// {
...
const char *SoundplaneModel::getHardwareStr() {
  ...
  case kDeviceHasIsochSync:
    ...
    snprintf(mHardwareStr, kMiscStringSize, "%s #%s, firmware %d.%d.%d",
             soundplaneModelName, serial_number.c_str(), a, b, c);
    break;
  default:
...
// 			else
// 			{
// 				MLConsole() << "SoundplaneModel::loadZonesFromString: out
// of zones!\n";
// 			}
// 		}
// 		pNode = pNode->next;
...
TouchArray
SoundplaneModel::getTestTouchesFromTracker(time_point<system_clock> now) {
...
void SoundplaneModel::enableOutput(bool b) { mOutputEnabled = b; }

void SoundplaneModel::clear() { mTracker.clear(); }
