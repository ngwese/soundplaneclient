...
// MLPublishedParam: a parameter of one of the Procs in a DSP graph that is
// settable through the plugin wrapper. There are two concepts wrapped up in
// this that should be factored out. One is a way to view and manipulate a
// Property. There is a list of these ways. The other is publishing properties
// of multiple procs in a graph as one settable entity. the Property might be a
// string, so there is no way to route it to multiple procs within the graph. if
// there were, we could remove the "alsosets" and use proc connections.
// "alsosets" should check to see if Properties are of compatible types... sort
// of an ugliness point.
...
// ----------------------------------------------------------------
// a published param means: named parameter mParam of mProc is called
// mPublishedAlias.
//
...
private:
  class ParamAddress {
  public:
    ParamAddress(const ml::Path &alias, const ml::Symbol name)
        : procAddress(alias), paramName(name) {}
    ~ParamAddress() {}

    // procAddress is where to send the param.  can resolve to a single MLProc,
    // or a list of processors in the case of multiples.  The address is always
    // relative to the container that publishes the parameters.
    ml::Path procAddress;
    ml::Symbol paramName;
  };

public:
  MLPublishedParam(const ml::Path &address, const ml::Symbol name,
                   const ml::Symbol alias, const ml::Symbol type, int idx);
  ~MLPublishedParam();

  void setRange(float low, float high, float interval, bool log, float zt,
                float offset);
