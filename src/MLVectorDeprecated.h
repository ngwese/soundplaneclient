...
struct MLV4 {
  float f[4];
  bool operator==(const MLV4 &b) const { return (*f == *b.f); }
};
...
public:
  ...
  inline MLVec &operator+=(const MLVec &b) {
    val.v = _mm_add_ps(val.v, b.val.v);
    return *this;
  }
  inline MLVec &operator-=(const MLVec &b) {
    val.v = _mm_sub_ps(val.v, b.val.v);
    return *this;
  }
  inline MLVec &operator*=(const MLVec &b) {
    val.v = _mm_mul_ps(val.v, b.val.v);
    return *this;
  }
  inline MLVec &operator/=(const MLVec &b) {
    val.v = _mm_div_ps(val.v, b.val.v);
    return *this;
  }
  inline const MLVec operator-() const {
    return MLVec(_mm_sub_ps(_mm_setzero_ps(), val.v));
  }
  ...
  inline void clear() { val.f[0] = val.f[1] = val.f[2] = val.f[3] = 0.f; }
  inline void set(float f) { val.f[0] = val.f[1] = val.f[2] = val.f[3] = f; }
  inline MLVec &operator+=(const MLVec &b) {
    val.f[0] += b.val.f[0];
    val.f[1] += b.val.f[1];
    val.f[2] += b.val.f[2];
  ...
  inline const MLVec operator+(const MLVec &b) const {
    return MLVec(*this) += b;
  }
  inline const MLVec operator-(const MLVec &b) const {
    return MLVec(*this) -= b;
  ...
  inline MLVec &operator*=(const float f) {
    (*this) *= MLVec(f);
    return *this;
  }
  inline const MLVec operator*(const float f) const {
    return MLVec(*this) *= MLVec(f, f, f, f);
  }
  inline const MLVec operator/(const float f) const {
    return MLVec(*this) /= MLVec(f, f, f, f);
  }
...
inline const MLVec vmin(const MLVec &a, const MLVec &b) {
  return MLVec(_mm_min_ps(a.val.v, b.val.v));
}
inline const MLVec vmax(const MLVec &a, const MLVec &b) {
  return MLVec(_mm_max_ps(a.val.v, b.val.v));
}
inline const MLVec vclamp(const MLVec &a, const MLVec &b, const MLVec &c) {
  return vmin(c, vmax(a, b));
}
...
inline const MLVec vmin(const MLVec &a, const MLVec &b) {
  return MLVec(
      std::min(a.val.f[0], b.val.f[0]), std::min(a.val.f[1], b.val.f[1]),
      std::min(a.val.f[2], b.val.f[2]), std::min(a.val.f[3], b.val.f[3]));
}
inline const MLVec vmax(const MLVec &a, const MLVec &b) {
  return MLVec(
      std::max(a.val.f[0], b.val.f[0]), std::max(a.val.f[1], b.val.f[1]),
...
inline const MLVec vsqrt(const MLVec &a) {
  return MLVec(sqrt(a.val.f[0]), sqrt(a.val.f[1]), sqrt(a.val.f[2]),
               sqrt(a.val.f[3]));
}
...
inline float length(LineSegment a) { return (a.end - a.start).magnitude(); }
...
public:
  ...
  inline bool contains(const Vec2 &p) const {
    return (ml::within(p.x(), left(), right()) &&
            ml::within(p.y(), top(), bottom()));
  }
  ...
  inline void stretchWidth(float d) {
    val.f[0] -= d * 0.5f;
    val.f[2] += d;
  }
  inline void stretchHeight(float d) {
    val.f[1] -= d * 0.5f;
    val.f[3] += d;
  }

  inline void stretchWidthTo(float w) {
    float d = w - width();
    stretchWidth(d);
  }
  inline void stretchHeightTo(float h) {
    float d = h - height();
    stretchHeight(d);
  }

  inline void expand(float d) {
    stretchWidth(d);
    stretchHeight(d);
  }
  inline void expand(const Vec2 &b) {
    stretchWidth(b.x());
    stretchHeight(b.y());
  }
  ...
  inline bool contains(int px, int py) const {
    return (ml::within(px, (int)left(), (int)right()) &&
            ml::within(py, (int)top(), (int)bottom()));
  }
  inline void setBounds(int l, int t, int w, int h) {
    *this = MLRect((float)l, (float)t, (float)w, (float)h);
  }
