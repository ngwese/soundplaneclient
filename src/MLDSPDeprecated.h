...
// const uintptr_t kMLProcessChunkBits = 6;     // signals are always processed
// in chunks of this size. const uintptr_t kMLProcessChunkSize = 1 <<
// kMLProcessChunkBits; const uintptr_t kMLProcessChunkVectors =
// kMLProcessChunkSize << kMLSamplesPerSSEVectorBits;
...
float scaleForRangeTransform(float a, float b, float c,
                             float d); // TODO replace with MLRange object
...
inline float fsin1(const float x) { return x - (x * x * x * 0.15f); }
...
inline __m128 log2Approx4(__m128 x) {
  ...
  p = POLY5(m, 3.1157899f, -3.3241990f, 2.5988452f, -1.2315303f, 3.1821337e-1f,
            -3.4436006e-2f);
...
public:
  MLRange()
      : mA(0.f), mB(1.f), mScale(1.f), mOffset(0.f), mClip(false),
        mMinOutput(0), mMaxOutput(0) {}
  MLRange(float a, float b)
      : mA(a), mB(b), mScale(1.f), mOffset(0.f), mClip(false), mMinOutput(0),
        mMaxOutput(0) {}
  MLRange(float a, float b, float c, float d, bool clip = false)
      : mA(a), mB(b), mScale(1.f), mOffset(0.f), mClip(clip), mMinOutput(0),
        mMaxOutput(0) {
  ...
  float operator()(float f) const {
    ...
    if (mClip)
      r = ml::clamp(r, mMinOutput, mMaxOutput);
    return r;
  }

  inline float convert(float f) const { return f * mScale + mOffset; }
...
inline ml::Matrix lerp(const ml::Matrix &b, const ml::Matrix &c,
                       const ml::Matrix &m) {
...
inline ml::Matrix lerpBipolar(const ml::Matrix &a, const ml::Matrix &b,
                              const ml::Matrix &c, const ml::Matrix &m) {
...
public:
  ...
  // MLTEST to DSPVector
  // use an interpolated coefficient matrix that must contain input width x 5
  // rows
  inline ml::Matrix operator()(const ml::Matrix &in, const ml::Matrix &coeffs) {
    ...
    for (int n = 0; n < frames; ++n) {
      ...
      const float out =
          pa0[n] * x + pa1[n] * x1 + pa2[n] * x2 + pb1[n] * y1 + pb2[n] * y2;
      x2 = x1;
      x1 = x;
      y2 = y1;
      y1 = out;
...
public:
  ...
  inline float processSample(float x) {
    ...
    // for(int i=0; i<kOversample; ++i) // TODO verify this is unrolled
    {
      ...
      mBandState +=
          mOmega * (mInState - mLoState -
                    mBandState * mOneMinusQ); // SEMclip(mBandState, oneMinusQ);
    }
    return mBandState;
  }
...
// overflow. The frequency resolution of this counter is good enough for most
// uses: around 0.000005 Hz.
class MLSineOsc {
public:
  static const float kIntDomain, kRootX, kOneSixth, kRange, kDomain, kScale,
      kDomainScale, kPhaseInvScale, kFlipOffset;
  MLSineOsc() : mOmega32(0), mStep32(0) {}
  ~MLSineOsc() {}

  inline void setSampleRate(int sr) {
    mInvSrDomain = (float)kIntDomain / (float)sr;
  }
...
// this triangle generator makes a looping counter by letting a 32 bit word
// overflow. it's a simple triangle, not antialiased.
...
public:
  ...
  void resize(float duration);
  inline void setMixParams(float b, float ff, float fb) {
    mBlend = b;
    mFeedForward = ff;
    mFeedback = fb;
  }
  inline void setFixedDelay(float d) {
    mFixedDelayInSamples = (int)(d * (float)mSR);
  }
...
class MLFDN {
public:
  MLFDN() : mSize(0), mSR(44100), mFeedbackAmp(0), mFreqMul(0.925) {}
  ~MLFDN() {}
