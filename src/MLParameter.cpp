...
MLPublishedParam::MLPublishedParam(const ml::Path &procPath,
                                   const ml::Symbol name,
                                   const ml::Symbol alias,
                                   const ml::Symbol type, int idx)
    : mPublishedAlias(alias), mIndex(idx), mFlip(false), mAutomatable(true) {
...
void MLPublishedParam::setRange(float low, float high, float v, bool log,
                                float zt, float offset) {
  mRangeLo = low;
  mRangeHi = high;
  mInterval = v;
  mZeroThreshold = zt;
...
void MLPublishedParam::addAddress(const ml::Path &procPath,
                                  const ml::Symbol name) {
  mAddresses.push_back(ParamAddress(procPath, name));
}

float MLPublishedParam::getDefault(void) { return mDefault; }

void MLPublishedParam::setDefault(float val) { mDefault = val; }

float MLPublishedParam::getValue(void) { return mParamValue.getFloatValue(); }

const ml::Value &MLPublishedParam::getValueProperty() { return mParamValue; }
...
bool MLPublishedParam::getAutomatable(void) { return mAutomatable; }

void MLPublishedParam::setAutomatable(bool a) { mAutomatable = a; }
