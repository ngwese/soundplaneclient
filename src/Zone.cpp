...
void Zone::addTouchToFrame(int i, Touch t) {
  // MLTEST
  // debug() << "zone " << mName << " adding touch " << t << " at " << x << ", "
  // << y << "\n";
...
// after all touches for a frame have been received using addTouchToFrame,
// generate any needed messages about the frame and prepare for the next frame.
...
void Zone::processTouchesNoteRow(const std::bitset<kMaxTouches> &freedTouches) {
  for (int i = 0; i < kMaxTouches; ++i) {
    ...
    if (isActive && !wasActive) {
      ...
      float vibratoHP =
          (currentXPos - vibratoX) * mVibrato * kSoundplaneVibratoAmount;
...
// process any note offs. called by the model for all zones before
// processTouches() so that any new notes with the same index as an expiring one
// will have a chance to get started.
//
// TODO do we need freedTouches at all if we are re-sorting the touches later in
// instruments and always have say 16 possible touches?
